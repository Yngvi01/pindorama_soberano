  ...
  useEffect(() => {
    const fetchMoletons = async () => {
      try {
        ...
        console.error('Erro ao buscar moletons')
      ...
      <div className="container mx-auto px-4 py-16">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-4">
          {moletonsFiltrados.map((moletom, index) => (
            <Link href={`/produtos/${moletom.id}`} key={moletom.id}>
              <motion.div
                initial={{ opacity: 0, y: 30 }}
                animate={{ opacity: 1, y: 0 }}
                transition={{ duration: 0.6, delay: index * 0.05 }}
                className="bg-white rounded-xl shadow-lg hover:shadow-xl transition-all duration-300 group overflow-hidden border border-gray-100 relative cursor-pointer"
              >
              {/* Badge de desconto */}
              {moletom.desconto && (
